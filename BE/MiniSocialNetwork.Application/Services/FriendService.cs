...
public sealed class FriendService : IFriendService
{
    ...
    // NEW: calls repository, returns raw recommendation response DTOs
    public async Task<IReadOnlyCollection<RecommendationResponse>> RecommendAsync(string userId, int take = 20)
    {
        var result = await _repo.GetRecommendationsAsync(userId, take);
        return result;
    }
}
