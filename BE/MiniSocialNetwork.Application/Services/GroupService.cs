...
public class GroupService : IGroupService
{
    ...
    string requesterId)
    {
        ...
        var newOwner = group.Members
            .FirstOrDefault(x => x.UserId == newOwnerId);

        if (newOwner == null)
            throw new InvalidOperationException(
                "Member not found");

        var currentOwner = group.Members
            .First(x => x.UserId == requesterId);

        // Owner -> Member
        currentOwner.Role = (int)GroupRole.Member;

        // Member/Admin -> Owner
        newOwner.Role = (int)GroupRole.Owner;

        // đổi OwnerId của Group
