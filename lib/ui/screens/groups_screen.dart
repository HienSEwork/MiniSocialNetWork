        ...
        actions: [
          if (context.watch<AuthProvider>().session?.userId ==
            ...
            IconButton(
              ...
              onPressed: () => showGroupMembersSheet(context, widget.group),
...
Future<void> showGroupMembersSheet(
  ...
  var membersFuture = LocalDataService.instance.get(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError) {
                    return FriendlyState(
                      ...
                      title: 'Chưa tải được thành viên',
                      message: '${snapshot.error}',
                    );
                  }
                      ...
                      return Card(
                        child: ListTile(
                          leading: UserAvatar(
                            ...
                            imageUrl: member['avatarUrl']?.toString(),
                          ),
                          title: Text('${member['displayName']}'),
                          subtitle: Text(isOwner ? 'Chủ nhóm' : role),
                          trailing: isOwner
                              ...
                              : PopupMenuButton<String>(
                                  onSelected: (action) async {
                                    try {
                                      if (action == 'remove') {
                                        await LocalDataService.instance.delete(
                                          '/groups/${group.id}/members/${member['id']}',
                                        );
                                      } else {
                                        await LocalDataService.instance.put(
                                          '/groups/${group.id}/members/${member['id']}/role',
                                          data: {
                                            'role': action == 'moderator'
                                                ? 1
                                                : 2,
                                          },
                                        );
                                      ...
                                      setSheetState(
                                        () => membersFuture = LocalDataService
                                            .instance
                                  ...
                                  itemBuilder: (_) => const [
                                    PopupMenuItem(
                                      value: 'moderator',
                                      child: Text('Đặt làm điều phối'),
                                    ),
                                    PopupMenuItem(
                                      value: 'member',
                                      child: Text('Đặt làm thành viên'),
                                    ),
                                    PopupMenuItem(
                                      value: 'remove',
                                      child: Text('Mời khỏi nhóm'),
                                    ),
                                  ],
                                ),
                        ),
