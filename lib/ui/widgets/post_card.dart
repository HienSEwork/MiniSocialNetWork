                ...
                Expanded(
                  child: FutureBuilder<List<SocialComment>>(
                    ...
                    builder: (_, snapshot) {
                      ...
                      final currentUserId = sheetContext
                          .read<AuthProvider>()
                          .session
                          ?.userId;
                      if (items.isEmpty) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          return ListTile(
                            ...
                            trailing: item.userId != currentUserId
                                ? null
                                : PopupMenuButton<String>(
                                    tooltip: 'Tùy chọn bình luận',
                                    onSelected: (action) async {
                                      if (action == 'edit') {
                                        final next = await _editCommentDialog(
                                          sheetContext,
                                          item.content,
                                        );
                                        if (next == null) return;
                                      ...
                                      final refreshed = provider.getComments(
                                        post.id,
                                      );
                                      setSheetState(
                                        () => commentsFuture = refreshed,
                                      );
                                    },
                                    itemBuilder: (_) => const [
                                      PopupMenuItem(
