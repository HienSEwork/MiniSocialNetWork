...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> _restoreSession() async {
    ...
    if (userId != null) {
      ...
      if (!isGuest && token?.startsWith('local-session:') != true) {
  ...
  Future<String?> login(String email, String password) async {
    ...
    try {
      ...
      if (token.isEmpty || userId.isEmpty) {
        return _t(
          'The local account data has an unexpected format.',
          'Dữ liệu tài khoản cục bộ chưa đúng định dạng.',
        );
      }
      await _saveSession(
        UserSession(
      ...
      return error.statusCode == 404
          ? _t(
              'Local sign-in data is unavailable. You can use guest mode.',
              'Dữ liệu đăng nhập cục bộ chưa sẵn sàng. Bạn có thể dùng chế độ khách.',
            )
          : error.message;
    } finally {
      _setLoading(false);
  ...
  Future<String?> register(
    ...
    try {
      ...
      return error.statusCode == 404
          ? _t(
              'Local registration is unavailable.',
              'Đăng ký cục bộ chưa sẵn sàng.',
            )
          : error.message;
    } finally {
      _setLoading(false);
