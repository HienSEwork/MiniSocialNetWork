...
class _GroupDetailScreenState extends State<GroupDetailScreen> {
  ...
  late Future<SocialGroup> _groupFuture;
  late SocialGroup _group;
  @override
  void initState() {
    super.initState();
    _group = widget.group;

    _posts = context.read<CommunityProvider>().fetchGroupPosts(_group);
    _groupFuture =
        context.read<CommunityProvider>().getGroup(widget.group.id);
  }
  Future<void> _deleteGroup(BuildContext context) async {
    ...
    if (error == null) {
      ...
      Navigator.pop(context); // quay về màn Groups
    } else {
      showResultMessage(
        context,
        error,
  ...
  Future<void> _reload() async {
    ...
    final group = await provider.getGroup(_group.id);
    final posts = await provider.fetchGroupPosts(group);
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => ConversationScreen(group: _group),
              ),
            ),
...
class _GroupHero extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final copy = AppCopy.of(context);
    final currentUserId =
        context.watch<AuthProvider>().session?.userId;

    final isMember =
    group.members.any((e) => e.userId == currentUserId);
                ...
                Row(
                  children: [
                    ...
                    OutlinedButton(
                      onPressed: () async {
                        ...
                        if (isMember) {
                          if (isOwner) {
                            showResultMessage(
                              context,
                              "Bạn phải chuyển quyền sở hữu trước khi rời nhóm.",
                              error: true,
                            );
                            return;
                          }
                          ...
                          error = await provider.leaveGroup(group.id);
                        } else {
                          error = await provider.joinGroup(group);
                        }
                      ...
                      child: Text(isMember ? "Rời nhóm" : copy.join),
                    ),
                  ],
                ),
              ],
